  ...
  return (
    <MuiAppBar position="sticky" color="primary">
      <Tabs
        ...
        variant="fullWidth"
      >
        <Tab label={!isXS && "Summary"} icon={<DetailIcon fontSize={25} />} />
        <Tab label={!isXS && "QR Code"} icon={<QRCodeIcon fontSize={25} />} />
        <Tab label={!isXS && "Actions"} icon={<ActionIcon fontSize={25} />} />
        <Tab label={!isXS && "History"} icon={<HistoryIcon fontSize={25} />} />
      </Tabs>
    </MuiAppBar>
  );
}
